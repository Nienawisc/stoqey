...
export class MarketDataResolver {
  ...
  async getMarketData(
    ...
    try {
        ...
        const gotCandles = await mkdapi.getCandles(symbol, start || new Date(), end, range || '1m');
  ...
  @Query(() => MarketDataType)
  async getQuote(@Arg("symbol") symbol: string): Promise<MarketDataType | null> {
    try {
        const mkdapi = new MarketDataAPI();
  ...
  async getInfo(@Arg("symbol") symbol: string): Promise<MarketSymbolInfo | null> {
    try {
        const mkdapi = new MarketDataAPI();
