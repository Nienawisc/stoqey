...
import { signupApi, SignupCreds } from './api';
...
const SignUp: React.FC<any> = (props: any) => {
  ...
  async function signup(args: SignupCreds) {
      ...
      <View style={styles.background}>
        <View style={[styles.rect2, { backgroundColor: Colors.white }]}>
          <View style={styles.progressBarColumn}>
            ...
            <>
              <Text style={[styles.text, { paddingBottom: 60 }]}>CREATE ACCOUNT</Text>
              <PhoneVerification
              ...
              {token && (
                <FormComponent
                  ...
                  onSubmit={async (data: any) => {
                    const { password, passwordRepeat, email, fullname } = data;
                    ...
                    if (!validateEmail(email)) {
                      return showToast('Email is not valid', false);
                    }

                    const { token: firebaseToken, phone } = state;
                    return await signup({
                      firebaseToken,
                      phone,
                      fullname,
